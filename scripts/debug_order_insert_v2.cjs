...
(async function(){
  ...
  console.log('sign=', sign.error || 'ok');
  if(sign.error) { console.error(sign.error); process.exit(1); }
  const session = sign.data.session;
  console.log('session tokens found?', !!session?.access_token, !!session?.refresh_token);
  const set = await client.auth.setSession(session.access_token ? { access_token: session.access_token, refresh_token: session.refresh_token } : {});
  console.log('setSession err=', set.error);
  const current = await client.auth.getSession();
  console.log('current session', current.error || 'ok', current.data?.session?.user?.email);
  const order = {
    id: 'ORD-DEBUG2-' + Date.now(),
